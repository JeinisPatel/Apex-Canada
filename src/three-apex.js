...
const raw = [[700,1500],[1500,1480],[2200,1430],[2520,1180],[2580,800],[2520,520],[1900,380],[1300,420],[950,640],[880,1000],[620,1280],[520,1600],[560,1980],[760,2120],[700,2360],[900,2520],[1280,2470],[1560,2560],[1760,2430],[2040,2520],[2380,2500],[2620,2300],[2700,2020],[2540,1880],[2180,1900],[1880,1820],[1560,1900],[1240,1830],[980,1920],[760,1760]];
const SCALE = 0.17;
...
function loop(){const dt=Math.min(clock.getDelta(),.033); cars.forEach(c=>updateCar(c,dt)); updateCamera(dt); speedEl.textContent=Math.round(player.speed*3.1).toString().padStart(3,'0'); lapEl.textContent=Math.min(player.lap,3); statusEl.textContent=player.lap>3?'RUN COMPLETE':'HARBOUR 3D SHAKEDOWN'; renderer.render(scene,camera); requestAnimationFrame(loop);}
